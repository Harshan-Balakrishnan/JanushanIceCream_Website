    ...
    <section className="builder section" id="build" aria-labelledby="builder-title" style={{ "--builder-flavour": activeFlavour.colour, "--builder-flavour-deep": activeFlavour.deep, "--builder-sauce": activeSauce.colour } as React.CSSProperties}>
      ...
      <div className="builder-layout">
        <div className="builder-controls" aria-label="Ice cream builder controls">
          <fieldset><legend><span>01</span> Choose your serve</legend><div className="builder-options builder-options-four">{bases.map((item) => <button type="button" key={item.id} className={base === item.id ? "is-active" : ""} onClick={() => setBase(item.id)}><i>{item.icon}</i><strong>{item.name}</strong></button>)}</div></fieldset>
          <fieldset><legend><span>02</span> Pick a flavour</legend><div className="builder-options flavour-choice-row">{flavours.map((item) => <button type="button" key={item.id} className={flavour === item.id ? "is-active" : ""} onClick={() => setFlavour(item.id)}><i className="flavour-dot" style={{ background: item.colour }} /><strong>{item.name}</strong></button>)}</div></fieldset>
          <fieldset><legend><span>03</span> How many scoops?</legend><div className="scoop-counter" role="group" aria-label="Number of scoops">{[1, 2, 3].map((count) => <button type="button" key={count} className={scoops === count ? "is-active" : ""} onClick={() => setScoops(count)}>{count}<small>{count === 1 ? "SCOOP" : "SCOOPS"}</small></button>)}</div></fieldset>
