        ...
        {selected && (
          <motion.div className="product-modal-backdrop" role="presentation" initial={reduceMotion ? { opacity: 1 } : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
            <motion.div className="product-modal" role="dialog" aria-modal="true" aria-labelledby="product-modal-title" initial={reduceMotion ? false : { y: 38, opacity: 0, scale: 0.96 }} animate={reduceMotion ? undefined : { y: 0, opacity: 1, scale: 1 }} exit={reduceMotion ? undefined : { y: 22, opacity: 0, scale: 0.98 }} transition={{ type: "spring", stiffness: 210, damping: 24 }} style={{ "--product-glow": selected.glow } as React.CSSProperties}>
              ...
              <div className="modal-visual"><div className="modal-halo" aria-hidden="true" /><span className="modal-order-badge">JANUSHAN FAVOURITE</span><Image src={selected.image} alt={selected.name} fill sizes="(max-width: 800px) 90vw, 48vw" className="modal-product-image" unoptimized={selected.image.startsWith("http")} /></div>
              <div className="modal-copy"><p className="section-kicker">{selected.eyebrow}</p><h3 id="product-modal-title">{selected.name}</h3><div className="modal-price">Rs. {formatter.format(selected.price)}/-</div><p>{selected.blurb}</p><div className="modal-order-note"><span aria-hidden="true">✓</span><span>Ready to order? Send this item to Janushan on WhatsApp.</span></div><div className="modal-actions"><a className="button button-primary modal-order-button" href={orderHref(selected)} target="_blank" rel="noreferrer"><span>Order on WhatsApp</span><span aria-hidden="true">↗</span></a><a className="button button-ghost" href="#flavours" onClick={() => setSelected(null)}>Explore Flavours</a></div><small className="modal-future">Availability and final order details can be confirmed directly with Janushan.</small></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
